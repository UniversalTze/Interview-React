...
- run `npm run dev`
- Ensure that you also go into ready-set-hire/backend and run `docker compose up -d`. (This application only works when you have a backend server feeding it information. I've made a simple one to handle this.). Check here {Link to Read Me}! 
...
Example prompts: 
`explain {this code} and how its affecting the webpage`
`Can you see where the code is going wrong here: {code}`
`What boostrap component would be best to use for this feature`
`Using a card vs a button in a Link` 
